...
func TestConstants_值正确(t *testing.T) {
	...
	if ClientID != strings.TrimSpace(os.Getenv(AntigravityOAuthClientIDEnv)) {
		t.Errorf("ClientID 不匹配: got %s", ClientID)
	}
	secret, err := getClientSecret()
	if strings.TrimSpace(os.Getenv(AntigravityOAuthClientSecretEnv)) == "" {
		if err == nil {
			t.Fatalf("getClientSecret 未配置时应返回错误，实际 secret: %s", secret)
		}
	} else if err != nil {
		t.Fatalf("getClientSecret 已配置时不应报错: %v", err)
	}
