...
func TestResolveLogFilePath_WithDataDir(t *testing.T) {
	t.Setenv("DATA_DIR", "/tmp/LightBridge-data")
	got := resolveLogFilePath("")
	want := filepath.Join("/tmp/LightBridge-data", "logs", "LightBridge.log")
	if got != want {
...
func TestBuildFileCore_InvalidPathFallback(t *testing.T) {
	...
	opts.Output.FilePath = filepath.Join(os.DevNull, "logs", "LightBridge.log")
